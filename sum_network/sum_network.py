...
x = tf.placeholder(tf.float32, shape=[None, 40], name="Placeholder_x")
...
W_conv1 = weight_variable([5, 2, 1, 32])
b_conv1 = bias_variable([32])

# reshape x to a 4-d tensor
x_mat = tf.reshape(x, [-1, 10, 4, 1])
...
W_conv2 = weight_variable([5, 2, 32, 64])
...
W_conv3 = weight_variable([5, 2, 64, 128])
...
W_fc1 = weight_variable([4 * 10 * 128, 1024])
b_fc1 = bias_variable([1024])

h_pool3_flat = tf.reshape(h_pool3, [-1, 4*10*128])
...
for i in range(10001):
    batch_x, batch_y = training_data.next_batch(250)
    if i %100 == 0:
